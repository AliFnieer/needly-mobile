...
- State + data layers (real code lives here):
  - **Client state**: Zustand stores in `src/stores/` (e.g. `auth-store.ts`). Screens/components read store slices with hooks; mutate via store actions.
  - **Server state**: TanStack Query v5 (`@tanstack/react-query`). The `QueryClient` lives in `src/providers/query-provider.tsx` (wraps the whole app in `src/app/_layout.tsx`). Data-fetching lives in query hooks in `src/hooks/` (e.g. `use-auth.ts`); components never call the REST layer directly.
  - **HTTP**: Axios. The shared instance is created in `src/hooks/use-axios.ts` (`useAxios()`) with the base URL, Bearer injection, 401→refresh→retry-once interception, and `ApiError` normalization. Services (`src/services/auth-api.ts`) call `apiClient` directly; the request/response interceptors handle auth, refresh, and error shaping.
  - **Endpoints**: all REST paths are centralized in `src/utils/util.ts` (`API_ENDPOINTS`). Never hardcode a URL in a query hook or service — import the constant/helper.
- `src/features/`, `src/services/`, `src/providers/`, etc. are mostly **empty scaffolding** (only `.gitkeep`). The README's thorough architecture/feature docs are aspirational, not implemented — verify files exist before referencing them. Real code currently lives in `src/components/`, `src/hooks/`, `src/constants/theme.ts`, `src/app/`, `src/hooks/use-axios.ts`, `src/hooks/use-auth.ts`, `src/utils/util.ts`, `src/services/auth-api.ts`, `src/services/auth-token-store.ts`, `src/stores/auth-store.ts`, `src/features/auth/`, `src/providers/`.
...
- `.env` is gitignored with `.env*.local`; Expo inlines `EXPO_PUBLIC_API_URL` at build time, falling back to the dev URL in `src/hooks/use-axios.ts`. Never commit `.env`.
...
- **Base URLs**: dev `http://localhost:8080/api/v1`, prod `https://api.needly.com/api/v1`. The client reads `EXPO_PUBLIC_API_URL` (`process.env.EXPO_PUBLIC_API_URL`, inlined by Expo) and falls back to the dev URL — set in `src/hooks/use-axios.ts` (`API_BASE_URL`).
...
- **Endpoints implemented so far** (all defined in `src/utils/util.ts`, calls in `src/services/auth-api.ts`): `POST /auth/register`, `POST /auth/login`, `POST /auth/refresh`, `POST /auth/logout` (optional body `{refresh_token}`), `GET /auth/me`, `POST /auth/forgot-password`, `POST /auth/reset-password` (body `{token, new_password}`), `GET /auth/verify-email?token=`, `POST /auth/resend-verification` (requires auth, no body). Returned shapes: `AuthResponse = { access_token, refresh_token, token_type, expires_in, user }`, `User = { id, first_name, last_name, email, email_verified, created_at, updated_at }`.
- **Frontend flow** (matches the real API): signup → `register` stores tokens + redirects to the verify screen ("check your email") because registration does NOT auto-sign-in; login requires a verified email — unverified logins are routed to `/(auth)/verify`; forgot-password sends an email, the reset link carries the `token` in the URL and opens `/(auth)/reset-password` with that param. The OTP/code-entry screen stays but is disambiguated by `mode` (`signup` | `forgot`); `mode=signup` relies on the emailed 6-digit code from the authenticated `resend-verification` call, `mode=forgot` is a pass-through "check your email" state.
